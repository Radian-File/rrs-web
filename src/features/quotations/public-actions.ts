...
async function rateLimit(scope: string, token: string) { try { await assertRequestRateLimit(scope, 5, 30 * 60 * 1000, token); return null; } catch (error) { return recoverableError(error); } }
...
export async function requestQuotationRevisionAction(_: PublicQuotationActionState, formData: FormData): Promise<PublicQuotationActionState> {
  const limited = await rateLimit("quotation-revision", String(formData.get("token") ?? "")); if (limited) return limited;
  ...
  try { const data = parsed.data; await prisma.$transaction(async (tx) => { const quote = await tx.quotation.findUnique({ where: { publicTokenHash: hashPublicToken(data.token) } }); if (!quote || !quote.isCurrent || quote.publicTokenRevokedAt || !["SENT", "VIEWED"].includes(quote.status) || quote.validUntil < new Date()) throw new PublicQuotationUnavailableError("Quotation is no longer available for revision."); await tx.quotation.update({ where: { id: quote.id }, data: { status: "REVISION_REQUESTED", revisionRequests: { create: { requestedBy: data.requestedBy, reason: data.reason } }, activities: { create: { type: "REVISION_REQUESTED", description: "Client requested a quotation revision." } } } }); if (quote.inquiryId) await tx.inquiry.update({ where: { id: quote.inquiryId }, data: { status: "DISCUSSING" } }); }); } catch (error) { return recoverableError(error); }
...
export async function rejectQuotationAction(_: PublicQuotationActionState, formData: FormData): Promise<PublicQuotationActionState> {
  const limited = await rateLimit("quotation-reject", String(formData.get("token") ?? "")); if (limited) return limited;
  ...
  try { const data = parsed.data; await prisma.$transaction(async (tx) => { const quote = await tx.quotation.findUnique({ where: { publicTokenHash: hashPublicToken(data.token) } }); if (!quote || !quote.isCurrent || quote.publicTokenRevokedAt || !["SENT", "VIEWED"].includes(quote.status)) throw new PublicQuotationUnavailableError("Quotation cannot be rejected."); await tx.quotation.update({ where: { id: quote.id }, data: { status: "REJECTED", rejectedAt: new Date(), activities: { create: { type: "REJECTED", description: data.reason, metadata: { reason: data.reason } } } } }); if (quote.inquiryId) await tx.inquiry.update({ where: { id: quote.inquiryId }, data: { status: "REJECTED" } }); }); } catch (error) { return recoverableError(error); }
