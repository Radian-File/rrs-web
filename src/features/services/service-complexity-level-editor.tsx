  ...
  return (
    <form ref={formRef} action={action} onSubmit={capture} className="space-y-4 rounded-[16px] border border-border bg-surface-container/35 p-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-primary">{level.code}</p><p className="mt-1 text-xs text-secondary">Semua field di bawah tampil pada Client detail saat level dipublish.</p></div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="Nama level untuk Client" name="title" defaultValue={level.title} error={error("title")} />
        <Field label="Starting estimate yang ditampilkan (IDR)" name="startingPrice" type="number" min="0" step="1000" defaultValue={level.startingPrice ?? ""} required={false} error={error("startingPrice")} />
      </div>
      <Area label="Penjelasan level untuk Client" name="summary" defaultValue={level.summary} error={error("summary")} />
      <div className="grid gap-4 lg:grid-cols-2">
        <Area label="Indikator yang dilihat Client" hint="Satu indikator per baris." name="indicators" defaultValue={level.indicators.join("\n")} error={error("indicators")} />
        <Area label="Sinyal perlu scope lebih tinggi" hint="Satu sinyal per baris." name="escalationSignals" defaultValue={level.escalationSignals.join("\n")} error={error("escalationSignals")} />
      </div>
