...
export function ServiceDetailLevelGuide() {
  ...
  const copy = isId
    ? {
        ...
        title: "Pilih level yang paling mendekati kebutuhanmu.",
    ...
    : {
        ...
        title: "Choose the level closest to your project.",
  ...
  return <section className="border-t border-border py-10 md:py-14" aria-labelledby="service-level-title"><div className="grid gap-7 xl:grid-cols-[minmax(15rem,.52fr)_minmax(0,1.48fr)]"><div className="xl:sticky xl:top-28 xl:self-start"><p className="text-[9px] font-black uppercase tracking-[.18em] text-primary">{copy.eyebrow}</p><h2 id="service-level-title" className="mt-4 max-w-md font-display text-[clamp(2.8rem,4.8vw,5.3rem)] font-black uppercase leading-[.86] tracking-[-.065em]">{copy.title}</h2><p className="mt-5 max-w-md text-sm leading-7 text-secondary">{copy.description}</p></div><div className="overflow-hidden rounded-[24px] border border-white/10 bg-[radial-gradient(circle_at_78%_16%,rgba(200,237,115,.1),transparent_30%),linear-gradient(145deg,#242724,#171918)] p-4 text-white sm:p-6 lg:p-7"><p className="text-[9px] font-black uppercase tracking-[.18em] text-accent-lime">{copy.levels}</p><div role="tablist" aria-label={copy.levels} className="mt-4 flex snap-x gap-2 overflow-x-auto pb-2 lg:overflow-visible">{levels.map((level, index) => { const selected = level.code === activeLevel.code; return <button key={level.code} id={`service-level-${level.code}`} type="button" role="tab" aria-selected={selected} aria-controls="service-level-detail" tabIndex={selected ? 0 : -1} onClick={() => selectLevel(level.code)} onKeyDown={(event) => handleKeyDown(event, index)} className={cn("min-h-11 min-w-[8.5rem] snap-start rounded-[14px] border px-4 py-3 text-left transition-[background-color,border-color,transform] duration-200 motion-reduce:transition-none lg:flex-1", selected ? "border-accent-lime/45 bg-accent-lime text-background" : "border-white/10 bg-white/[.035] text-white hover:border-white/25 hover:bg-white/[.08]")}><span className="block text-[8px] font-black uppercase tracking-[.16em] opacity-65">Level {index + 1}</span><span className="mt-1 block font-display text-xl font-black">{level.title}</span></button>; })}</div><div id="service-level-detail" role="tabpanel" aria-live="polite" className="mt-5 rounded-[18px] border border-white/10 bg-black/15 p-5 sm:p-6"><div className="flex flex-col justify-between gap-5 border-b border-white/10 pb-5 sm:flex-row sm:items-end"><div><p className="text-[9px] font-black uppercase tracking-[.18em] text-accent-lime">{activeLevel.code}</p><h3 className="mt-3 font-display text-4xl font-black tracking-[-.055em]">{activeLevel.title}</h3><p className="mt-3 max-w-2xl text-sm leading-7 text-white/62">{activeLevel.summary}</p></div><div className="shrink-0"><p className="text-[8px] font-black uppercase tracking-[.15em] text-white/38">{copy.estimate}</p><p className="mt-2 font-display text-2xl font-black text-accent-lime">{activeLevel.estimate ?? "Custom scope"}</p></div></div><div className="mt-5 grid gap-5 sm:grid-cols-2"><DetailList icon={Check} title={copy.indicators} items={activeLevel.indicators} /><DetailList icon={CircleHelp} title={copy.escalation} items={activeLevel.escalationSignals} /></div></div></div></div></section>;
}
