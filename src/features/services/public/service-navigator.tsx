...
export function ServiceNavigator({
  ...
  return <PublicProductStage titleId="service-navigator-title" eyebrow={copy.eyebrow} index={String(groups.findIndex((group) => group.slug === activeGroup.slug) + 1).padStart(2, "0")} meta={activeGroup.name} frameClassName="bg-[radial-gradient(circle_at_82%_16%,rgba(200,237,115,.1),transparent_30%),linear-gradient(145deg,#302d1d,#1d201d_62%)]">
    <div className="grid gap-4 xl:grid-cols-[128px_minmax(260px,.72fr)_minmax(0,1.28fr)]">
      ...
      <section aria-live="polite" aria-labelledby="service-navigator-title" className="min-w-0 rounded-[22px] border border-white/10 bg-[#242724] p-5 sm:p-7 lg:p-8">
        <p className="break-words text-[9px] font-black uppercase tracking-[.18em] text-accent-lime">{copy.detailLabel} / {activeGroup.name}</p>
        ...
        <div className="mt-7 grid gap-4 border-t border-white/10 pt-6 sm:grid-cols-2"><div><p className="text-[8px] font-black uppercase tracking-[.15em] text-white/35">{copy.estimate}</p><p className="mt-2 break-words font-display text-2xl font-black text-accent-lime">{level?.estimate ?? service.estimate}</p></div><div><p className="flex items-center gap-2 text-[8px] font-black uppercase tracking-[.15em] text-white/35"><Clock3 className="size-3.5 text-accent-lime" aria-hidden="true" />{copy.timeline}</p><p className="mt-2 text-sm font-semibold text-white/72">{service.deliveryEstimate ?? copy.timelineFallback}</p></div></div>
