...
export async function createServiceAction(_state: ServiceActionState, formData: FormData): Promise<ServiceActionState> {
  ...
  const service = await prisma.service.create({ data: { ...parsed.data, startingPrice: parsed.data.startingPrice ?? null, coverImageUrl: parsed.data.coverImageUrl ?? null, deliveryEstimate: parsed.data.deliveryEstimate ?? null, revisionGuidance: parsed.data.revisionGuidance ?? null, currency: "IDR" } });
...
export async function updateServiceAction(_state: ServiceActionState, formData: FormData): Promise<ServiceActionState> {
  ...
  await prisma.service.update({ where: { id: current.id }, data: { ...parsed.data, startingPrice: parsed.data.startingPrice ?? null, coverImageUrl: parsed.data.coverImageUrl ?? null, deliveryEstimate: parsed.data.deliveryEstimate ?? null, revisionGuidance: parsed.data.revisionGuidance ?? null, currency: "IDR" } });
