...
export function ServiceEditor({ service, types, lockedServiceType }: { service?: EditableService; types: { id: string; name: string }[]; lockedServiceType?: { id: string; name: string } }) {
  ...
  return <form ref={formRef} action={action} onSubmit={capture} className="space-y-6">
    ...
    <section className="grid gap-5 md:grid-cols-2">
      ...
      {lockedServiceType ? <label><span className="mb-2 block text-sm font-semibold">Jenis layanan terkunci</span><input type="hidden" name="serviceTypeId" value={lockedServiceType.id}/><Input aria-label="Jenis layanan terkunci" value={lockedServiceType.name} readOnly className="bg-surface-container/60 text-secondary"/><span className="mt-2 block text-xs text-secondary">Dipilih dari workspace Jenis Layanan.</span></label> : <label><span className="mb-2 block text-sm font-semibold">Jenis layanan</span><select name="serviceTypeId" required defaultValue={service?.serviceTypeId ?? ""} className={fieldErrorClass(error("serviceTypeId"), "h-12 w-full rounded-[12px] border border-border bg-surface px-4 text-sm")}><option value="">Pilih jenis layanan</option>{types.map((type)=><option key={type.id} value={type.id}>{type.name}</option>)}</select><FieldError id="serviceTypeId-error" error={error("serviceTypeId")}/></label>}
