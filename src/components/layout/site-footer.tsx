...
export async function SiteFooter() {
  ...
  const groups = [
    ...
    { title: dictionary.footer.company, links: [[dictionary.nav.about, "/about"], ["Contact", "/contact"], ["Privacy", "/privacy"], ["Terms", "/terms"]] },
  ] as const;

  return (
    <footer className="border-t border-border bg-surface">
      <div className="mx-auto grid max-w-[1280px] gap-12 px-5 py-16 md:grid-cols-[1.4fr_repeat(3,1fr)] md:px-8 lg:px-16">
        <div><Brand /><p className="mt-4 max-w-sm text-sm leading-6 text-secondary">{dictionary.footer.description}</p></div>
