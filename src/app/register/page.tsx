    ...
    <main className="grid min-h-screen place-items-center px-5 py-12">
      <div className="w-full max-w-2xl">
        ...
        <Card><CardContent className="p-7 md:p-9">
          ...
          <div className="mt-8"><RegisterForm labels={{ fullName: dictionary.auth.fullName, whatsapp: dictionary.auth.whatsapp, email: dictionary.auth.email, company: dictionary.auth.company, password: dictionary.auth.password, confirmPassword: dictionary.auth.confirmPassword, create: dictionary.auth.createAccount, creating: dictionary.auth.creating }} /></div>
          <p className="mt-6 text-center text-sm text-secondary">{dictionary.auth.existing} <Link href="/login" className="font-semibold text-primary hover:underline">{dictionary.auth.signIn}</Link></p>
        </CardContent></Card>
      </div>
    </main>
  );
