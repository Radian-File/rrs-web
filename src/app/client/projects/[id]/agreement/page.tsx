...
import { AgreementAcceptanceControl } from "@/features/projects/recoverable-client-controls";
...
export default async function ClientAgreementPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ accepted?: string }> }) {
  ...
  return <>
    <WorkspacePageHeader eyebrow="Project agreement" title="Review before accepting." description="This private document records the scope, payment schedule, and collaboration terms for your project." actions={<Button asChild variant="outline"><Link href={`/client/projects/${project.id}`}>Back to project</Link></Button>}/>
    {accepted === "1" && <div role="status" className="mt-6 flex gap-3 rounded-[12px] border border-success/25 bg-success-soft p-4 text-sm text-success"><CheckCircle2 className="mt-0.5 size-5 shrink-0" /><p><strong>Agreement accepted.</strong> The project can now continue to the initial payment stage.</p></div>}
    <AgreementDocument agreement={project.agreement} project={project} snapshot={snapshot} locale={locale} />
    {pending && <Card className="mt-6 border-primary/25 bg-accent-soft"><CardContent><h2 className="font-display text-xl font-extrabold">Ready to accept this agreement?</h2><p className="mt-3 text-sm leading-6 text-secondary">By accepting, you confirm that you have reviewed this document, including the project scope, payment schedule, and collaboration terms.</p><AgreementAcceptanceControl projectId={project.id} agreementVersion={project.agreement.version}/></CardContent></Card>}
  </>;
}
