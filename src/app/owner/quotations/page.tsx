...
export default async function QuotationsPage() {
  ...
  return <><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-sm font-bold uppercase tracking-[.14em] text-primary">{dictionary.portal.quotations}</p><h1 className="mt-3 font-display text-3xl font-extrabold">Scope and price before commitment.</h1><p className="mt-2 text-sm text-secondary">{dictionary.common.activeRecords}</p></div><div className="flex flex-wrap gap-3"><Button asChild variant="outline"><Link href="/owner/quotations/archive"><Archive className="size-4"/>{dictionary.common.archiveRecords}</Link></Button><Button asChild><Link href="/owner/quotations/create"><Plus className="size-4"/>Create Quotation</Link></Button></div></div><form id="bulk-archive-form" action={bulkArchiveQuotationsAction} className="mt-5"><BulkArchiveControls allCount={await prisma.quotation.count({where:{isCurrent:true,archivedAt:null}})} entity="quotations"/><QuotationTable quotations={quotations} empty="No quotations yet." selectable/></form></>;
}

export function QuotationTable({ quotations, empty, archived = false, selectable = false }: { quotations: Awaited<ReturnType<typeof prisma.quotation.findMany>>; empty: string; archived?: boolean; selectable?: boolean }) {
