...
export default async function OwnerServicesPage({ searchParams }: { searchParams: Promise<{ page?: string; q?: string; catalog?: string; types?: string; services?: string; levels?: string; skipped?: string }> }) {
  const { page: rawPage, q: rawQuery, catalog, types: importedTypes, services: importedServices, levels: importedLevels, skipped } = await searchParams;
  const page = parsePage(rawPage);
  const q = rawQuery?.trim() ?? "";
  const filter = q ? { OR: [{ title: { contains: q, mode: "insensitive" as const } }, { slug: { contains: q, mode: "insensitive" as const } }, { category: { contains: q, mode: "insensitive" as const } }] } : {};
  ...
  return <>
    ...
    <PaginationControls pathname="/owner/services" page={page} hasNext={hasNext} params={{ q: q || undefined }}/>
  </>;
}
