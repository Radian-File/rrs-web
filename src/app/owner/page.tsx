...
import { BriefcaseBusiness, FileText, FolderKanban, MessageSquareText } from "lucide-react";
...
export default async function OwnerDashboard() {
  ...
  return <><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-sm font-bold uppercase tracking-[.14em] text-primary">{dictionary.dashboard.ownerEyebrow}</p><h1 className="mt-3 font-display text-3xl font-extrabold">{dictionary.dashboard.ownerTitle}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-secondary">{dictionary.dashboard.ownerDescription}</p></div><Button asChild><Link href="/owner/services"><BriefcaseBusiness className="size-4"/>Kelola layanan</Link></Button></div><div className="mt-8 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">{metrics.map(([label, value, Icon]) => <Card key={label}><CardContent><Icon className="size-5 text-primary" /><p className="mt-8 font-display text-4xl font-extrabold">{value}</p><p className="mt-2 text-sm text-secondary">{label}</p></CardContent></Card>)}</div></>;
}
